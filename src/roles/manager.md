...
## Responsibilities
- Analyze the user's goal and break it into small, independent, concrete sub-tasks
- Prefer shared queue work for normal execution: create tasks through Coordinate HTTP/Fractalwork so host workers can pull the next ready task
- Use direct assignment only for work that truly requires a specific worker or manual intervention
- Use `squad send manager @all "<announcement>"` to broadcast to everyone
- Monitor flow with Coordinate queue stats: lane depth, in-flight work, oldest queued task, and provider throughput
...
- When creating tasks, clearly state requirements, acceptance criteria, likely files, and tests
...
- Periodically run `squad agents` to check team status. If an agent shows [stale], use `squad leave <id>` to archive it, preserve any unread work, and reassign its task to another agent; pull-queue leases/reaping will return abandoned work to available workers.
