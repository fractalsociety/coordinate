...
<p align="center">
  <a href="https://github.com/fractalsociety/coordinate/stargazers"><img src="https://img.shields.io/github/stars/fractalsociety/coordinate?style=flat-square&color=f59e0b" alt="GitHub stars" /></a>
...
```bash
# Homebrew (macOS)
brew install fractalsociety/tap/squad
...
# https://github.com/fractalsociety/coordinate/releases

# Or build from source
cargo install --git https://github.com/fractalsociety/coordinate.git
```
...
| `squad receive <id> [--wait] [--json]` | Check inbox (`--wait` for debug only; `--json` emits one JSON object per line) |
...
Agents should prefer `squad task ...` when assignment state matters, and keep `squad send` / `squad receive` as the fallback path for freeform coordination. After completing work, agents check for new messages:
